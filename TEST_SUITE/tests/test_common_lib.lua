...
function TestRunner.assert_not_nil(value, message)
    if value ~= nil then
        ...
        table.insert(
            TestRunner.errors,
            "FAIL: " .. (message or "assertion") .. " - expected non-nil"
        )
        return false
    end
end
...
TestRunner.register("Module loading: client module loads with isClient", function()
    TestRunner.assert_not_nil(
        pz_commons_client,
        "pz_commons_client should be loaded (uses isServer check)"
    )
end)
...
TestRunner.register("pz_utils: Debounce.Call creates active debounce", function()
    ...
    TestRunner.assert_true(
        escape.Debounce.IsActive("test_call"),
        "debounce should be active after Call"
    )
end)
...
TestRunner.register("pz_utils: Debounce.Cancel", function()
    ...
    TestRunner.assert_false(
        escape.Debounce.IsActive("test_cancel"),
        "debounce should be inactive after cancel"
    )
end)
