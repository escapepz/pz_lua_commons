...
-- Usage example
local playerStatsEvent, addPlayerStatChange = createDebouncedEventProcessor("PlayerStats", "stats_debounce", 10)
...
function StateMachine.new(initialState)
    ...
    function sm:defineTransition(from, to, condition)
        ...
        table.insert(self.transitions[from], {to = to, condition = condition or function() return true end})
    end
...
function DeferredOperation.new(id, delayTicks)
    return {
        ...
        queue = function(self, operation)
            table.insert(self.queue, operation)
