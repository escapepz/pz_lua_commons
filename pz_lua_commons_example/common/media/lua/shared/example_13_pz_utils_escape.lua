...
safeLogger:log("Another info message", "info")
...
safeLogger:log("Debounce 'player_move' is active: " .. tostring(isActive), "INFO")
...
safeLogger:log("Cancelled debounce 'player_move'", "INFO")
...
safeLogger:log("Created event: OnPlayerDamage", "INFO")
...
safeLogger:log("Added 2 listeners to OnPlayerDamage event", "INFO")
safeLogger:log("Event has " .. playerDamageEvent:GetListenerCount() .. " listeners", "INFO")

-- Trigger the event
safeLogger:log("Triggering OnPlayerDamage event...", "INFO")
...
safeLogger:log("Disabled OnPlayerDamage event", "INFO")

-- Try triggering disabled event (won't trigger listeners)
playerDamageEvent:Trigger(50, "Hunter")
safeLogger:log("Event triggered but disabled - no listeners executed", "DEBUG")

-- Get event info
local eventInfo = escape.EventManager.getEventInfo("OnPlayerDamage")
safeLogger:log(
    "OnPlayerDamage info: enabled="
        .. tostring(eventInfo.enabled)
        .. ", listeners="
        .. eventInfo.listeners,
    "INFO"
)
...
safeLogger:log("Invalid module result: " .. tostring(invalidModule), "DEBUG")
...
safeLogger:log("Current IRL timestamp: " .. timestamp, "INFO")

-- This can be used to track time-based events
local startTime = escape.Utilities.GetIRLTimestamp()
safeLogger:log("Started timer at: " .. startTime, "DEBUG")
...
safeLogger:log("Added listener, count: " .. cleanupEvent:GetListenerCount(), "INFO")

-- Remove the listener
cleanupEvent:Remove(myListener)
safeLogger:log("Removed listener, count: " .. cleanupEvent:GetListenerCount(), "INFO")
